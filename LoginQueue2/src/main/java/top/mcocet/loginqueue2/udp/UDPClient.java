...
public class UDPClient {
    ...
    public BungeeMessenger.ServerStatus requestServerInfoViaMSLP(String targetIp, int targetPort) {
        ...
        if (isDebug()) {
            plugin.getLogger().info("MSLP [" + serverName + "] 开始检测，目标=" + ip + ":" + targetPort);
        }
        try (java.net.Socket socket = new java.net.Socket()) {
            ...
            if (isDebug()) {
                plugin.getLogger().info("MSLP [" + serverName + "] 响应: " + json.substring(0, Math.min(json.length(), 200)));
            }
            ...
            if (isDebug()) {
                plugin.getLogger().info("MSLP [" + serverName + "] 获取到服务器信息: " + status);
            }
            return status;
        } catch (Exception e) {
            if (isDebug()) {
                plugin.getLogger().warning("MSLP [" + serverName + "] 检测失败: " + e.getMessage());
            }
            return null;
        }
    }
