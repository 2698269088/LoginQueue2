...
public class UDPServer {
    ...
    private void handleServerList(String payload) {
        ...
        String encryptedPayload = payload.substring(sepIndex + 1);
        if (secretKey == null) {
            plugin.getLogger().warning("UDP 收到服务器列表但无密钥，无法解密。");
            return;
        }
        String decrypted;
        try {
            ...
            plugin.getLogger().warning("UDP 服务器列表解密失败: " + e.getMessage());
            return;
        }
        ...
        for (int i = 0; i < count; i++) {
            int nameIndex = 1 + i * 2;
            int displayIndex = nameIndex + 1;
            if (nameIndex >= parts.length) {
                break;
            }
            String name = parts[nameIndex];
            String displayName = displayIndex < parts.length ? parts[displayIndex] : "";
            servers.put(name, displayName);
        }

        if (plugin instanceof LoginQueue2Online) {
            ((LoginQueue2Online) plugin).updateRemoteServerList(servers);
            if (plugin.getConfig().getBoolean("debug", false)) {
                plugin.getLogger().info("UDP 已同步主插件服务器列表: " + servers.keySet());
            }
        }
    }
