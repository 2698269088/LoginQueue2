...
public class ConnectCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (plugin.isVirtualQueueEnabled()) {
            // 服务器存在性校验：优先匹配内部名，其次匹配显示名
            String resolved = plugin.resolveServerName(serverName);
            if (resolved == null) {
                sender.sendMessage("§c服务器 " + serverName + " 不存在。");
                return true;
            }
            serverName = resolved;
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            // 已同步主插件服务器列表时，内部名与显示名均可补全（内部名优先去重）
            if (plugin.isVirtualQueueEnabled() && plugin.hasRemoteServerList()) {
                Set<String> added = new HashSet<>();
                for (Map.Entry<String, String> entry : plugin.getRemoteServerList().entrySet()) {
                    if (entry.getKey().toLowerCase().startsWith(input) && added.add(entry.getKey())) {
                        completions.add(entry.getKey());
                    }
                    String displayName = entry.getValue();
                    if (displayName != null && !displayName.isEmpty()) {
                        // 去掉颜色代码后按前缀匹配，补全候选不带颜色代码
                        String plain = org.bukkit.ChatColor.stripColor(
                                org.bukkit.ChatColor.translateAlternateColorCodes('&', displayName));
                        if (plain.toLowerCase().startsWith(input) && added.add(plain)) {
                            completions.add(plain);
                        }
                    }
                }
                return completions;
            ...
            List<String> servers = plugin.getConfig().getStringList("udp-sync.server-list");
            if (servers.isEmpty()) {
                // 兼容旧版本顶层配置路径
                servers = plugin.getConfig().getStringList("server-list");
            }
