...
public class PluginMessageListener {
    ...
    public PluginMessageListener(LoginQueue2VC plugin, ProxyServer server, Logger logger) {
        this.plugin = plugin;
        this.server = server;
        this.logger = logger;
    }
    ...
    public void onPluginMessage(PluginMessageEvent event) {
        ...
        if (LoginQueue2VC.CHANNEL_CONNECT_OTHER.equals(channel)) {
            handleConnectOther(event);
            return;
        }

        if (LoginQueue2VC.CHANNEL_CONNECT_REQUEST.equals(channel)) {
            handleConnectRequest(event);
            return;
        }

        if (LoginQueue2VC.CHANNEL_SERVER_INFO.equals(channel)) {
        ...
        if (LoginQueue2VC.CHANNEL_LOGIN_SUCCESS.equals(channel)) {
        ...
        if (!targetServer.getPlayersConnected().isEmpty()) {
            ...
            relayPlayer.getCurrentServer().ifPresent(conn -> {
                    ...
                    conn.sendPluginMessage(
                            com.velocitypowered.api.proxy.messages.MinecraftChannelIdentifier.from(LoginQueue2VC.CHANNEL_SERVER_INFO),
                            out.toByteArray()
                    );
            });
        } else {
