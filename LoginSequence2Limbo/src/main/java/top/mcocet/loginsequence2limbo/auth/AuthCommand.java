...
public class AuthCommand implements CommandExecutor {
    ...
    public AuthCommand(LoginSequence2Limbo plugin, AuthManager authManager, PlayerJoinListener playerJoinListener) {
        this.plugin = plugin;
    ...
    public void execute(CommandSender sender, String[] args) {
        if (!(sender instanceof Player)) {
            sender.sendMessage("[LoginSequence] 该命令只能由玩家执行");
            return;
        }
        ...
        if (!authManager.isEnabled()) {
            player.sendMessage("[LoginSequence] 认证功能未启用");
            return;
        }
    ...
    private void handleRegister(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage("[LoginSequence] 用法: /register <密码> <确认密码>");
            return;
        }
        ...
        if (last != null && now - last < cooldown) {
            player.sendMessage("[LoginSequence] 请等待 " + ((cooldown - (now - last)) / 1000 + 1) + " 秒后再注册");
            return;
        }
        ...
        if (!password.equals(confirm)) {
            player.sendMessage("[LoginSequence] 两次输入的密码不一致");
            return;
        }
        ...
        if (password.length() < minLength) {
            player.sendMessage("[LoginSequence] 密码长度不能少于 " + minLength + " 个字符");
            return;
        }
        if (password.length() > maxLength) {
            player.sendMessage("[LoginSequence] 密码长度不能超过 " + maxLength + " 个字符");
            return;
        }

        if (authManager.isRegistered(player.getName())) {
            player.sendMessage("[LoginSequence] 你已经注册过了，请使用 /login 登录");
            return;
        }
        ...
        if (authManager.register(player.getName(), password, player.getName(), ip)) {
            ...
            Limbo.getInstance().getConsole().sendMessage("[LoginSequence] 玩家 " + player.getName() + " 注册成功");

            // 注册成功后自动登录
            authManager.updateLogin(player.getName(), ip);
            player.sendMessage("[LoginSequence] 注册成功！已自动登录。");
            ...
            if (autoQueue) {
                player.sendMessage("[LoginSequence] 正在进入排队队列...");
                playerJoinListener.addPlayerToQueue(player);
            } else {
                player.sendMessage("[LoginSequence] 请使用 /join 命令手动加入排队队列");
            }
        } else {
            player.sendMessage("[LoginSequence] 注册失败，请稍后再试");
        }
    }

    private void handleLogin(Player player, String[] args) {
        if (args.length < 1) {
            player.sendMessage("[LoginSequence] 用法: /login <密码>");
            return;
        }
        ...
        if (last != null && now - last < cooldown) {
            player.sendMessage("[LoginSequence] 请等待 " + ((cooldown - (now - last)) / 1000 + 1) + " 秒后再尝试");
            return;
        }

        if (!authManager.isRegistered(player.getName())) {
            player.sendMessage("[LoginSequence] 你还没有注册，请使用 /register <密码> <确认密码> 注册");
            return;
        }
        ...
        if (authManager.checkPassword(player.getName(), password)) {
            ...
            String ip = player.clientConnection != null && player.clientConnection.getInetAddress() != null
                    ...
                    : "unknown";
            authManager.updateLogin(player.getName(), ip);
            player.sendMessage("[LoginSequence] 登录成功！");
            player.setTitleSubTitle("§a登录成功", "§e欢迎回来！", 10, 70, 20);
            Limbo.getInstance().getConsole().sendMessage("[LoginSequence] 玩家 " + player.getName() + " 登录成功");
            ...
            loginCooldown.put(player.getUniqueId(), now);
            player.sendMessage("[LoginSequence] 密码错误，请重试");
        }
    }

    private void handleChangePassword(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage("[LoginSequence] 用法: /changepassword <旧密码> <新密码>");
            return;
        }

        if (!authManager.isRegistered(player.getName())) {
            player.sendMessage("[LoginSequence] 你还没有注册");
            return;
        }
        ...
        if (!authManager.checkPassword(player.getName(), oldPassword)) {
            player.sendMessage("[LoginSequence] 旧密码错误");
            return;
        }
        ...
        if (newPassword.length() < minLength) {
            player.sendMessage("[LoginSequence] 新密码长度不能少于 " + minLength + " 个字符");
            return;
        }
        if (newPassword.length() > maxLength) {
            player.sendMessage("[LoginSequence] 新密码长度不能超过 " + maxLength + " 个字符");
            return;
        }

        if (authManager.changePassword(player.getName(), newPassword)) {
            player.sendMessage("[LoginSequence] 密码修改成功");
        } else {
            player.sendMessage("[LoginSequence] 密码修改失败");
        }
    }
}
