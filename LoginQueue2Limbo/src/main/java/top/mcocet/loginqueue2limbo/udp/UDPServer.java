...
public class UDPServer implements PlayerJoinListener.VirtualQueueHandler {
    ...
    public UDPServer(LoginQueue2Limbo plugin, BungeeMessenger messenger, PlayerJoinListener playerJoinListener, int port) {
        this.plugin = plugin;
    ...
    public void start() {
        ...
        try {
            ...
            executor = Executors.newSingleThreadExecutor(r -> {
                Thread t = new Thread(r, "LS2L-UDP-Server");
                t.setDaemon(true);
                return t;
            });
            executor.submit(this::listen);

            log(languageManager.getLogMessage("udp-server-started", "port", String.valueOf(port)));

            // 注册虚拟队列处理器
            playerJoinListener.setVirtualQueueHandler(this);

            // 启动服务器列表与队列状态广播任务
            long interval = plugin.getConfigValueLong("udp-sync.connect-queue.status-interval", 3) * 20L;
            if (interval > 0) {
            ...
            log(languageManager.getLogMessage("udp-server-start-failed", "port", String.valueOf(port), "error", e.getMessage()));
        }
    }
    ...
    public void stop() {
        ...
        log(languageManager.getLogMessage("udp-server-stopped"));
    }
    ...
    private void listen() {
        while (running && socket != null && !socket.isClosed()) {
            try {
                ...
                if (running) {
                    log(languageManager.getLogMessage("udp-server-receive-error", "error", e.getMessage()));
                }
            }
        }
    }
    ...
    private void handlePacket(DatagramPacket packet) {
        ...
        try {
            ...
            log(languageManager.getLogMessage("udp-server-parse-error", "error", e.getMessage()));
            return;
        }
        ...
        if (sepIndex < 0) {
            if (plugin.isDebug()) {
                log(languageManager.getLogMessage("udp-server-invalid-data", "data", rawData.substring(0, Math.min(rawData.length(), 20))));
            }
            return;
        }
        ...
        if (plugin.isDebug()) {
            log(languageManager.getLogMessage("udp-server-received", "type", type, "ip", packet.getAddress().getHostAddress(), "port", String.valueOf(packet.getPort())));
        }
        ...
        switch (type) {
            ...
            default:
                if (plugin.isDebug()) {
                    log(languageManager.getLogMessage("udp-server-unknown-type", "type", type));
                }
                break;
        }
    }
    ...
    private void handleConnectRequest(String payload) {
        ...
        if (sepIndex < 0) {
            log(languageManager.getLogMessage("udp-server-request-format-error"));
            return;
        }
        ...
        if (client == null) {
            log(languageManager.getLogMessage("udp-server-client-not-found", "server", serverName));
            return;
        }
        ...
        try {
            ...
            log(languageManager.getLogMessage("udp-server-decrypt-failed", "server", serverName, "error", e.getMessage()));
            return;
        }
        ...
        if (parts.length < 3) {
            log(languageManager.getLogMessage("udp-server-request-data-error", "data", decrypted));
            return;
        }
        ...
        try {
            ...
            log(languageManager.getLogMessage("udp-server-invalid-uuid", "uuid", parts[0]));
            return;
        }
        ...
        if (plugin.isDebug()) {
            log(languageManager.getLogMessage("udp-server-connect-request",
    ...
    private void handleConnectCancel(String payload) {
        ...
        if (plugin.isDebug()) {
            log(languageManager.getLogMessage("udp-server-connect-cancel", "uuid", playerUuid.toString(), "source", serverName));
        }
    }
    ...
    private void sendConnectResponse(UDPClient client, UUID uuid, boolean success, int position, int online, int max, String message) {
        ...
        try {
            ...
            log(languageManager.getLogMessage("udp-server-encrypt-failed", "server", client.getServerName(), "error", e.getMessage()));
            return;
        }
    ...
    private void broadcastServerList() {
        ...
        for (UDPClient client : messenger.getUdpClients()) {
            ...
            try {
                ...
                if (plugin.isDebug()) {
                    log("UDP 服务器列表加密失败 (" + client.getServerName() + "): " + e.getMessage());
                }
                continue;
            }
    ...
    private void broadcastQueueStatus() {
        ...
        for (UUID uuid : virtualUuids) {
            ...
            try {
                ...
                if (plugin.isDebug()) {
                    log(languageManager.getLogMessage("udp-server-status-encrypt-failed", "server", sourceServer, "error", e.getMessage()));
                }
                continue;
            }
    ...
    public void onVirtualPlayerAllowed(UUID uuid, String targetServer, String sourceServer) {
        UDPClient client = getUDPClient(sourceServer);
        if (client == null) {
            log(languageManager.getLogMessage("udp-server-allow-no-client", "server", sourceServer));
            return;
        }
        String secretKey = client.getSecretKey();
        if (secretKey == null || secretKey.isEmpty()) {
            log(languageManager.getLogMessage("udp-server-allow-no-key", "server", sourceServer));
            return;
        }
        ...
        try {
            ...
            log(languageManager.getLogMessage("udp-server-allow-encrypt-failed", "server", sourceServer, "error", e.getMessage()));
            return;
        }
        ...
        if (plugin.isDebug()) {
            log(languageManager.getLogMessage("udp-server-player-allowed", "uuid", uuid.toString(), "server", targetServer, "source", sourceServer));
        }
    }
