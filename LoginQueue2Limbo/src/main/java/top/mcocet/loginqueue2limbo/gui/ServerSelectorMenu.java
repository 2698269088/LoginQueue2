...
public class ServerSelectorMenu implements Listener {
    ...
    private List<ServerEntry> buildServerEntries() {
        ...
        if (servers != null && !servers.isEmpty()) {
            for (Map<?, ?> map : servers) {
                ...
                entries.add(new ServerEntry(name, resolveDisplayName(map, name), status));
            }
        } else {
            // 兼容旧版单服务器配置
            String mainServer = plugin.getConfigValueString("queue.main-server", "main");
            BungeeMessenger.ServerStatus status = messenger != null ? messenger.getServerStatus(mainServer) : null;
            entries.add(new ServerEntry(mainServer, mainServer, status));
        }
     ...
     * 未配置或留空时返回内部名本身
     */
    private String resolveDisplayName(Map<?, ?> serverMap, String internalName) {
        Object displayObj = serverMap.get("display-name");
    ...
    private static class ServerEntry {
        ...
        ServerEntry(String name, String displayName, BungeeMessenger.ServerStatus status) {
            this.name = name;
